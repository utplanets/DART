...
!
! $Id: model_mod.f90 11694 2017-06-02 21:39:25Z nancy@ucar.edu $
...
! Assimilation interface for PlanetWRF model
!> \defgroup wrf model_mod
!> Model mod
!>
!> Model mod for PlanetWRF
!> @{
!-----------------------------------------------------------------------
!
!     interface for PlanetWRF
!
!-----------------------------------------------------------------------
!---------------- m o d u l e   i n f o r m a t i o n ------------------
!-----------------------------------------------------------------------

use         types_mod,   only : r8, i8, deg2rad, missing_r8, &
                                pi,      &
                                digits12, gas_constant_v

use  time_manager_mod,   only : time_type, set_time, set_calendar_type, GREGORIAN_MARS,&
                                set_date, get_date, SOLAR_MARS
...
!CL
                                QTY_SURFACE_ALBEDO, QTY_SURFACE_EMISSIVITY, &
                                QTY_DUST_OPACITY_7MB, QTY_THC!, &
!                                QTY_TRACER_1_MIXING_RATIO, QTY_TRACER_2_MIXING_RATIO, &
!                                QTY_ICE_CORE_MASS_CONCENTR
...
use default_model_mod,   only : init_conditions, init_time, nc_write_model_vars
...
use mpi_utilities_mod,   only : all_reduce_min_max, my_task_id, task_count
...
public ::  get_number_domains,          &
           ...
           get_planet_wrf_date, set_planet_wrf_date, &
           vert_convert,                &
           height_diff_check
...
character(len=256), parameter :: source   = &
   "$URL: https://svn-dares-dart.cgd.ucar.edu/DART/releases/Manhattan/models/wrf/model_mod.f90 $"
character(len=32 ), parameter :: revision = "$Revision: 11694 $"
character(len=128), parameter :: revdate  = "$Date: 2017-06-02 17:39:25 -0400 (Fri, 02 Jun 2017) $"
...
!integer :: calendar_type        = GREGORIAN_MARS
integer :: calendar_type        = SOLAR_MARS
...
logical :: polar = .true.         ! wrap over the poles
logical :: periodic_x = .true.    ! wrap in longitude or x
...
!CL NOT constants for Mars
real (kind=r8) :: ps0, ts0, gas_constant, heat_capacity_p, gravity, kappa
...
TYPE wrf_static_data_for_dart
   ...
   integer :: type_u10, type_v10, type_t2, type_th2, type_q2, &
              ...
              type_smois, type_2dflash, type_reff_dust2d

   integer :: type_albedo, type_emissivity, type_dust_opacity_7mb, type_thc
   integer :: type_trc01, type_trc02, type_trcic
...
!----------------------------------------------------------------------
if (debug) write(*,*) "static_init_model"
...
! problems that it might cause.
   ...
   wrf%dom(id)%type_reff_dust2d    = get_type_ind_from_type_string(id,'REFF_DUST2D')
   ...
   !CL planetary
   wrf%dom(id)%type_albedo           = get_type_ind_from_type_string(id,'ALBEDO')
   wrf%dom(id)%type_emissivity       = get_type_ind_from_type_string(id,'EMISS')
   wrf%dom(id)%type_dust_opacity_7mb = get_type_ind_from_type_string(id,'TAU_OD2D')
   wrf%dom(id)%type_thc              = get_type_ind_from_type_string(id,'THC')
   wrf%dom(id)%type_trc01             = get_type_ind_from_type_string(id,'TRC01')
   wrf%dom(id)%type_trc02             = get_type_ind_from_type_string(id,'TRC02')
   wrf%dom(id)%type_trcic             = get_type_ind_from_type_string(id,'TRC_IC')
...
if (debug) write(*,*) "get_state_meta_data"
...
if (debug) write(*,*) "model_interpolate"
id = 1
...
! JPH --- scm is only defined for d1
   ...
   if (id==0) then
      istatus = 1
      if (debug) write(*,*) "id is 0, returning"
      return
   endif
...
!      return
   ...
   !cl planet
   ! 2.a ALBEDO -- Surface Albedo (Christopher Lee)
   ! 2.b EMISS -- Surface Emissivity (Christopher Lee)
   ! 2.c TAU_OD2d -- Total dust opacity (Christopher Lee)
   ! 2.d THC -- Thermal Conductivity (Christopher Lee)
   ! 2.e REFF_DUST2D  (Christopher Lee)
...
!       obs_kind == QTY_ICE_CORE_MASS_CONCENTR .or. &
!       obs_kind == QTY_TRACER_1_MIXING_RATIO .or. &
!       obs_kind == QTY_TRACER_2_MIXING_RATIO
) then
...
!          obs_kind == QTY_TRACER_2_MIXING_RATIO
   ...
   elseif ( obs_kind == QTY_TEMPERATURE ) then
      ...
      !print*, 'k ', k
!      write(*,*) "-",task_count(), my_task_id(),"=", state_handle%id_num
      if(.not. surf_var) then
...
!*****************************************************************************
   ...
   !   XLAND is not in the dart_ind vector, so get it from wrf%dom(id)%land
      ...
      !-----------------------------------------------------
      ! 2.a Surface Albedo (ALBEDO)

      ! Surface albedo observation
   else if( obs_kind == QTY_SURFACE_ALBEDO ) then
     ...
     if ( boundsCheck( i, wrf%dom(id)%periodic_x, id, dim=1, type=wrf%dom(id)%type_albedo ) .and. &
           boundsCheck( j, wrf%dom(id)%polar,      id, dim=2, type=wrf%dom(id)%type_albedo ) ) then

          call getCorners(i, j, id, wrf%dom(id)%type_albedo, ll, ul, lr, ur, rc )
          if ( rc .ne. 0 ) &
            print*, 'model_mod.f90 :: model_interpolate :: getCorners ALBEDO rc = ', rc

          ! Interpolation for the ALBEDO field
          ill = get_dart_vector_index(ll(1),ll(2), 1, domain_id(id), wrf%dom(id)%type_albedo)
          iul = get_dart_vector_index(ul(1),ul(2), 1, domain_id(id), wrf%dom(id)%type_albedo)
          ilr = get_dart_vector_index(lr(1),lr(2), 1, domain_id(id), wrf%dom(id)%type_albedo)
          iur = get_dart_vector_index(ur(1),ur(2), 1, domain_id(id), wrf%dom(id)%type_albedo)
   ...
   ! 2.b Surface Emissivity (EMISS)
   ! Surface emissivity observation
 else if( obs_kind == QTY_SURFACE_EMISSIVITY ) then
     ! Check to make sure retrieved integer gridpoints are in valid range -- since the
     !   altimeter obs_def code has calls to surface pressure is it, then surf_obs must
     !   equal to .true. in order to use altimeter obs.
!cl     if ( boundsCheck( i, wrf%dom(id)%periodic_x, id, dim=1, type=wrf%dom(id)%TYPE_EMISSIVITY ) .and. &
!cl          boundsCheck( j, wrf%dom(id)%polar,      id, dim=2, type=wrf%dom(id)%TYPE_EMISSIVITY ) ) then
!cl
!cl       call getCorners(i, j, id, wrf%dom(id)%type_emissivity, ll, ul, lr, ur, rc )
!cl       if ( rc .ne. 0 ) &
!cl           print*, 'model_mod.f90 :: model_interpolate :: getCorners EMISS rc = ', rc
!cl
!cl       ! Interpolation for the emissivity field
!cl       ill = get_dart_vector_index(ll(1),ll(2), 1, domain_id(id), wrf%dom(id)%type_emissivity)
!cl       iul = get_dart_vector_index(ul(1),ul(2), 1, domain_id(id), wrf%dom(id)%type_emissivity)
!cl       ilr = get_dart_vector_index(lr(1),lr(2), 1, domain_id(id), wrf%dom(id)%type_emissivity)
!cl       iur = get_dart_vector_index(ur(1),ur(2), 1, domain_id(id), wrf%dom(id)%type_emissivity)
!cl       
!cl       fld(1,:) = dym*( dxm*x(ill) + dx*x(ilr) ) + dy*( dxm*x(iul) + dx*x(iur) )
!cl
!cl       ! If the boundsCheck functions return an unsatisfactory integer index, then set
!cl       !   fld as missing data
!cl     else
!cl       fld(1,:) = missing_r8
!cl     end if
!cl   !-----------------------------------------------------
   ! 2.c Total Dust Opacity (TAU_OD2D)
   ! Surface emissivity observation
 else if( obs_kind == QTY_DUST_OPACITY_7MB ) then
     ! Check to make sure retrieved integer gridpoints are in valid range -- since the
     !   altimeter obs_def code has calls to surface pressure is it, then surf_obs
     !   equal to .true. in order to use altimeter obs.
     if ( boundsCheck( i, wrf%dom(id)%periodic_x, id, dim=1, type=wrf%dom(id)%type_dust_opacity_7mb ) .and. &
           boundsCheck( j, wrf%dom(id)%polar,      id, dim=2, type=wrf%dom(id)%type_dust_opacity_7mb ) ) then
          ...
          ! Interpolation for the ALBEDO field
          ill = get_dart_vector_index(ll(1),ll(2), 1, domain_id(id), wrf%dom(id)%type_dust_opacity_7mb)
          iul = get_dart_vector_index(ul(1),ul(2), 1, domain_id(id), wrf%dom(id)%type_dust_opacity_7mb)
          ilr = get_dart_vector_index(lr(1),lr(2), 1, domain_id(id), wrf%dom(id)%type_dust_opacity_7mb)
          iur = get_dart_vector_index(ur(1),ur(2), 1, domain_id(id), wrf%dom(id)%type_dust_opacity_7mb)
   ...
   !-----------------------------------------------------
   ! 2.d Thermal Conductivity (THC)
   ! Surface emissivity observation
  else if( obs_kind == QTY_THC ) then
     ! Check to make sure retrieved integer gridpoints are in valid range -- since the
     !   altimeter obs_def code has calls to surface pressure is it, then surf_obs must
     !   equal to .true. in order to use altimeter obs.
!cl     if ( boundsCheck( i, wrf%dom(id)%periodic_x, id, dim=1, type=wrf%dom(id)%type_thc ) .and. &
!cl          boundsCheck( j, wrf%dom(id)%polar,      id, dim=2, type=wrf%dom(id)%type_thc ) ) then
!cl          call getCorners(i, j, id, wrf%dom(id)%type_thc, ll, ul, lr, ur, rc )
!cl          if ( rc .ne. 0 ) &
!cl              print*, 'model_mod.f90 :: model_interpolate :: getCorners THC rc = ', rc
!cl
!cl          ! Interpolation for the DUST field
!cl          ill = get_dart_vector_index(ll(1),ll(2), 1, domain_id(id), wrf%dom(id)%type_thc)
!cl          iul = get_dart_vector_index(ul(1),ul(2), 1, domain_id(id), wrf%dom(id)%type_thc)
!cl          ilr = get_dart_vector_index(lr(1),lr(2), 1, domain_id(id), wrf%dom(id)%type_thc)
!cl          iur = get_dart_vector_index(ur(1),ur(2), 1, domain_id(id), wrf%dom(id)%type_thc)
!cl
!cl          fld(1,:) = dym*( dxm*x(ill) + dx*x(ilr) ) + dy*( dxm*x(iul) + dx*x(iur) )
!cl
!cl      ! If the boundsCheck functions return an unsatisfactory integer index, then set
...
!   else if( obs_kind == QTY_REFF_DUST2D ) then
!     ! make sure vector includes the needed field
!     if ( wrf%dom(id)%type_reff_dust2d >= 0 ) then
!        call surface_interp_distrib(fld, wrf, id, i, j, obs_kind, wrf%dom(id)%type_reff_dust2d, dxm, dx, dy, dym, ens_size, state_handle)
!        if (all(fld == missing_r8)) goto 200
!     endif
!
   else
...
if (debug) write(*,*) "convert_vertical_state"
istatus = 0
...
!planet
integer :: PS0VarID, TS0VarID, GAS_CONSTANTVarID, HEAT_CAPACITY_PVarID, &
           GRAVITYVarID
...
if (debug) write(*,*) "nc_write_model_atts"
...
! wgl - added these to go with the I/O above (static_init_model) on model constants
! wgl - note that PS0 and TS0 have zeros, not o's
call nc_check(nf90_def_var(ncid, name="PS0", xtype=nf90_real, &
              dimids= DomDimID, varid=PS0VarID), &
              'nc_write_model_atts','def_var PS0')
call nc_check(nf90_put_att(ncid, PS0VarID, "long_name", &
              "Reference pressure level"), &
              'nc_write_model_atts','put_att PS0 long_name')
call nc_check(nf90_put_att(ncid, PS0VarID, "units","Pa"), &
              'nc_write_model_atts','put_att PS0 units')

call nc_check(nf90_def_var(ncid, name="TS0", xtype=nf90_real, &
              dimids= DomDimID, varid=TS0VarID), &
              'nc_write_model_atts','def_var TS0')
call nc_check(nf90_put_att(ncid, TS0VarID, "long_name", &
              "Reference temperature level"), &
              'nc_write_model_atts','put_att TS0 long_name')
call nc_check(nf90_put_att(ncid, TS0VarID, "units","K"), &
              'nc_write_model_atts','put_att TS0 units')

call nc_check(nf90_def_var(ncid, name="GAS_CONSTANT", xtype=nf90_real, &
              dimids= DomDimID, varid=GAS_CONSTANTVarID), &
              'nc_write_model_atts','def_var GAS_CONSTANT')
call nc_check(nf90_put_att(ncid, GAS_CONSTANTVarID, "long_name", &
              "Ideal gas constant for atmosphere"), &
              'nc_write_model_atts','put_att GAS_CONSTANT long_name')
call nc_check(nf90_put_att(ncid, GAS_CONSTANTVarID, "units","J kg^-1 K^-1"), &
              'nc_write_model_atts','put_att GAS_CONSTANT units')

call nc_check(nf90_def_var(ncid, name="HEAT_CAPACITY_P", xtype=nf90_real, &
              dimids= DomDimID, varid=HEAT_CAPACITY_PVarID), &
              'nc_write_model_atts','def_var HEAT_CAPACITY_P')
call nc_check(nf90_put_att(ncid, HEAT_CAPACITY_PVarID, "long_name", &
              "Heat capacity for constant pressure"), &
              'nc_write_model_atts','put_att HEAT_CAPACITY_P long_name')
call nc_check(nf90_put_att(ncid, HEAT_CAPACITY_PVarID, "units","J kg^-1 K^-1"), &
              'nc_write_model_atts','put_att HEAT_CAPACITY_P units')

call nc_check(nf90_def_var(ncid, name="GRAVITY", xtype=nf90_real, &
              dimids= DomDimID, varid=GRAVITYVarID), &
              'nc_write_model_atts','def_var GRAVITY')
call nc_check(nf90_put_att(ncid, GRAVITYVarID, "long_name", &
              "Acceleration of gravity"), &
              'nc_write_model_atts','put_att GRAVITY long_name')
call nc_check(nf90_put_att(ncid, GRAVITYVarID, "units","m s^-2"), &
              'nc_write_model_atts','put_att GRAVITY units')
...
! wgl - added these to go with the I/O above (static_init_model) on model constants
! wgl - note that PS0 and TS0 have zeros, not o's
! wgl - will the below always work?  ps0, ts0, g_c, h_c_p, and grav are in module
!         storage, so assuming that static_init_model has been run, these values
!         ought to be available in memory -- however, everything else being written
!         here is in the wrf / meta_data structures.
call nc_check(nf90_put_var(ncid,             PS0VarID, ps0), &
              'nc_write_model_atts','put_var ps0')
call nc_check(nf90_put_var(ncid,             TS0VarID, ts0), &
              'nc_write_model_atts','put_var ts0')
call nc_check(nf90_put_var(ncid,    GAS_CONSTANTVarID, gas_constant), &
              'nc_write_model_atts','put_var gas_constant')
call nc_check(nf90_put_var(ncid, HEAT_CAPACITY_PVarID, heat_capacity_p), &
              'nc_write_model_atts','put_var heat_capacity_p')
call nc_check(nf90_put_var(ncid,         GRAVITYVarID, gravity), &
              'nc_write_model_atts','put_var gravity')
...
if (debug) write(*,*) "get_model_pressure_profile_distrib"
...
real(kind=r8) :: rd_over_rv, cpovcv, znu_k

integer(i8) :: iqv !< I think this is i for index
integer(i8) :: it !< change to array
real(r8) :: qvf1(ens_size),rho(ens_size), x_iqv(ens_size), x_it(ens_size), ps(ens_size)

model_pressure_t_distrib = missing_r8

rd_over_rv = gas_constant / gas_constant_v
cpovcv = heat_capacity_p / (heat_capacity_p - gas_constant)

!Hydrostatic, using surface pressure and znw
!no water vapor
...
ps = model_pressure_surface_distrib(i, j, id, state_handle, ens_size)
znu_k = wrf%dom(id)%znu(k)
model_pressure_t_distrib =  (ps-wrf%dom(id)%p_top) * znu_k + wrf%dom(id)%p_top

!write(*,*) "pressure: ", i,j,k,ps, wrf%dom(id)%p_top,  znu_k, model_pressure_t_distrib
end function model_pressure_t_distrib

!#######################################################

function model_pressure_surface_distrib(i, j, id, state_handle,ens_size)

! compute pressure at surface at mass point

integer,             intent(in) :: i,j,id, ens_size
type(ensemble_type), intent(in) :: state_handle

real(r8)              :: model_pressure_surface_distrib(ens_size)
...
if ( wrf%dom(id)%type_mu < 0 .and. wrf%dom(id)%type_ps < 0 ) then
  call error_handler(E_ERR, 'model_pressure_surface:', &
...
if ( wrf%dom(id)%type_ps >= 0 ) then
   ...
   model_pressure_surface_distrib = x_ips

else
   imu = get_dart_vector_index(i,j,1, domain_id(id), wrf%dom(id)%type_mu)
   x_imu = get_state(imu, state_handle)
   model_pressure_surface_distrib = wrf%dom(id)%p_top + wrf%dom(id)%mub(i,j) + x_imu

endif

end function model_pressure_surface_distrib
...
model_pressure_s_distrib = scalar(model_pressure_surface_distrib(i, j, id, state_handle, 1))
...
real(r8) :: ps(ens_size), znu_k
integer(i8) :: it
real(r8)    :: ph_e(ens_size), x_it(ens_size)
real(kind=r8) :: rdovcp

model_rho_t_distrib(:) = missing_r8

rdovcp = gas_constant / heat_capacity_p
...
ps = model_pressure_t_distrib(i, j, k,id, state_handle, ens_size)
!ps=0
znu_k = wrf%dom(id)%znu(k)
!write(*,*) "density: ", i,j,k,ps,(ps/ps0),rdovcp, ps ,gas_constant,(ts0+x_it)

model_rho_t_distrib(:) = (ps0/ps)**rdovcp * ps /(gas_constant*(ts0+x_it))
!write(*,*) "density: ", i,j,k,ps,(ps/ps0),rdovcp, ps ,gas_constant,(ts0+x_it),model_rho_t_distrib
...
real(r8), allocatable :: x_ill(:), x_ilr(:), x_iul(:), x_iur(:)
if (debug) write(*,*) "get_model_height_profile_distrib"
allocate(x_ill(ens_size), x_ilr(ens_size), x_iul(ens_size), x_iur(ens_size))
...
if (debug) write(*,*) "read_wrf_dimensions"
...
integer :: itmp
logical, parameter    :: debug = .false.
if (debug) write(*,*) "read_wrf_file_attributes"
! get meta data and static data we need
   ...
   call nc_check( nf90_get_att(ncid, nf90_global, 'MAP_PROJ', itmp), &
                     'static_init_model', 'get_att MAP_PROJ')
   if(debug) write(*,*) ' map_proj was originally ',itmp,' but we changed to ',map_cassini
   wrf%dom(id)%map_proj = map_cassini
...
!   if(debug) write(*,*) ' map_proj is ',wrf%dom(id)%map_proj
   ...
   !wgl - get further metadata needed for planetWRF -- items stored as global attributes,
   !   whereas for Earth they can be accessed from module types_mod
   ! P0
   call nc_check( nf90_get_att(ncid, nf90_global, 'P0', ps0), &
                     'static_init_model', 'get_att P0' )
   if(debug) write(*,*) ' ps0 is ',ps0
   ! T0
   call nc_check( nf90_get_att(ncid, nf90_global, 'T0', ts0), &
                     'static_init_model', 'get_att T0' )
   if(debug) write(*,*) ' ts0 is ',ts0
   ! R_D
   call nc_check( nf90_get_att(ncid, nf90_global, 'R_D', gas_constant), &
                     'static_init_model', 'get_att R_D' )
   if(debug) write(*,*) ' gas_constant is ',gas_constant
   ! CP
   call nc_check( nf90_get_att(ncid, nf90_global, 'CP', heat_capacity_p), &
                     'static_init_model', 'get_att CP' )
   if(debug) write(*,*) ' heat_capacity_p is ',heat_capacity_p
   ! G
   call nc_check( nf90_get_att(ncid, nf90_global, 'G', gravity), &
                     'static_init_model', 'get_att G' )
   if(debug) write(*,*) ' gravity is ',gravity

   kappa = gas_constant / heat_capacity_p
   if(debug) write(*,*) ' kappa is ',kappa
...
logical, parameter    :: debug = .false.
if (debug) write(*,*) "assign_boundary_conditions"
...
logical, parameter    :: debug = .false.
integer               :: var_id 
if (debug) write(*,*) "read_wrf_static_data"
...
if (debug) write(*,*) "setup_map_projection"
...
logical :: debug = .false.
if (debug) write(*,*) "get_number_of_wrf_variables"
...
! the bounds and instructions if they exist
   ...
   integer :: ivar
   logical :: debug = .true.
   if (debug) write(*,*) "get_variable_bounds"
   wrf_varname_trim = ''
   ...
   do while ( trim(bounds_table(1,ivar)) /= 'NULL' ) 
     ...
     if ( bounds_varname_trim == wrf_varname_trim ) then
        if (debug) write(*,*) bounds_table(1:num_bounds_table_columns,ivar) 
...
integer               :: idim
   ...
   stagger = ''
if (debug) write(*,*) "get_variable_size_from_file"
! get variable ID
...
! type for this domain.  returns -1 if not found
   ...
   if (debug) write(*,*) "get_type_ind_from_type_string"
...
! mars has no months
month=1
read(tstring, '(i4,x,i5,3(x,i2))') year, day, hour, minute, second
...
!#######################################################
! planet_wrf has format for variable Times:
!
!0....5....0....5....| len=19
! YYYY-DDDDD_HH:MM:SS

subroutine set_planet_wrf_date (timestring, year, day, hour, minute, second)
...
character(len=4)  :: ch_year
character(len=5)  :: ch_day
character(len=2)  :: ch_hour, ch_minute, ch_second

write(ch_year,'(i4.4)') year
write(ch_day,'(i5.5)') day
write(ch_hour,'(i2.2)') hour
write(ch_minute,'(i2.2)') minute
write(ch_second,'(i2.2)') second
...
end subroutine set_planet_wrf_date

!#######################################################

subroutine get_planet_wrf_date (tstring, year, day, hour, minute, second)

implicit none
!--------------------------------------------------------
! Returns integers taken from wrf%timestring
! For Planetary, it is assumed that the tstring char array is as YYYY-DDDDD_hh:mm:ss

integer,           intent(out) :: year, day, hour, minute, second
character(len=19), intent(in)  :: tstring

read(tstring(1:4),'(i4)') year
read(tstring(6:10),'(i5)') day
read(tstring(12:13),'(i2)') hour
read(tstring(15:16),'(i2)') minute
read(tstring(18:19),'(i2)') second

end subroutine get_planet_wrf_date
...
!else if( ( obs_kind == QTY_TRACER_1_MIXING_RATIO )     .and. ( wrf%dom(id)%type_trc01 >= 0 ) ) then
!   part_of_state_vector = .true.
!   wrf_type =  wrf%dom(id)%type_trc01
!else if( ( obs_kind == QTY_TRACER_2_MIXING_RATIO )     .and. ( wrf%dom(id)%type_trc02 >= 0 ) ) then
!   part_of_state_vector = .true.
!   wrf_type =  wrf%dom(id)%type_trc02
!else if( ( obs_kind == QTY_ICE_CORE_MASS_CONCENTR )     .and. ( wrf%dom(id)%type_trcic >= 0 ) ) then
!   part_of_state_vector = .true.
!   wrf_type =  wrf%dom(id)%type_trcic
else
...
call set_planet_wrf_date(timestring, year, day, hour, minute, second)
...
! $Id: model_mod.f90 11694 2017-06-02 21:39:25Z nancy@ucar.edu $
! $Revision: 11694 $
! $Date: 2017-06-02 17:39:25 -0400 (Fri, 02 Jun 2017) $
